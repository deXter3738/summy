...
let shouldUseIconFallback = null;
...
function replacePhosphorWithFallback() {
  ...
  icons.forEach((icon) => {
    ...
    const fallback = phosphorFallbackMap[iconName] || "•";
    const fallbackIcon = document.createElement("span");
    fallbackIcon.className = "icon icon-fallback";
    fallbackIcon.setAttribute("aria-hidden", "true");
    if (icon.title) fallbackIcon.setAttribute("title", icon.title);
...
function hydrateIconFallback() {
  ...
  return fontsReady.then(() => {
    ...
    probe.className = "ph ph-house icon";
    probe.style.cssText = "position:fixed;left:-9999px;top:0;width:1px;height:1px;overflow:hidden;opacity:0;";
    document.body.appendChild(probe);

    const pseudoContent = window.getComputedStyle(probe, "::before").content;
    const hasPhosphorStyles = pseudoContent && pseudoContent !== "none" && pseudoContent !== "normal" && pseudoContent !== '""';
    const hasPhosphorFont = window.document.fonts ? window.document.fonts.check("16px Phosphor") : false;
    shouldUseIconFallback = !(hasPhosphorStyles && hasPhosphorFont);

    if (probe.parentElement) probe.remove();
  });
}

function syncFallbackIcons() {
  if (shouldUseIconFallback) {
    replacePhosphorWithFallback();
  }
}

function renderListTools() {
  const openButton = byId("open-filter-btn");
  const sortButton = byId("sort-order-btn");
  if (openButton) {
    const openCount = state.activeFilter === "attention" ? "Open" : "All";
    openButton.querySelector("span").textContent = `${openCount}`;
  }
  if (sortButton) {
    sortButton.querySelector("span").textContent = state.newestFirst ? "Newest" : "Oldest";
  }
}
...
document.querySelectorAll(".list-tools button").forEach((button, index) => {
  button.addEventListener("click", () => {
    ...
    if (index === 0) {
      ...
      const openLabel = button.querySelector("span");
      if (openLabel) {
        openLabel.textContent = state.activeFilter === "attention" ? "Open" : "All";
      }
    } else {
      state.newestFirst = !state.newestFirst;
      const sortLabel = button.querySelector("span");
      if (sortLabel) {
        sortLabel.textContent = state.newestFirst ? "Newest" : "Oldest";
      }
    }
    const firstMatch = conversations.find(matchesCurrentView);
    if (firstMatch) state.activeConversationId = firstMatch.id;
